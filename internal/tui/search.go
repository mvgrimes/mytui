...
func rowMatchesQuery(res *Result, rowIdx int, query string) bool {
	if res.DbResult == nil || rowIdx < 0 || rowIdx >= len(res.DbResult.Rows) {
		return false
	}
	lowerQuery := strings.ToLower(query)
	for _, cell := range interfaceSliceToStrings(res.DbResult.Rows[rowIdx]) {
...
func findMatchingRow(res *Result, query string, startRow int, forward bool) int {
